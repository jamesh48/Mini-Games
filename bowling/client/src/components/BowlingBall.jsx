...
export default ({rolling}) => {
  ...
  const [holePositions, setHolePositions] = useState(styles);
    ...
    if (rolling === true) {
      window.rollingInterval = setInterval(() => {
        setHolePositions((eHP) => {
          ...
          let ballShadow = { ...eHP.ballShadow, ...{ boxShadow: `inset -25px ${currentShadow + 'px'} 40px rgba(0,0,0,.5)`, bottom: currentPosition +'px' } };
  ...
  return (
    <div id ='ball-element' onClick={() => { setRolling(true) }}>

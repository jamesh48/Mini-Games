...
export default () => {
  ...
  const [{ flippers, mines, numbers, scrollBoard, colorDelay }, boardDispatch] =
    useBoardContext();
  ...
  BoardHooks.resetGameEffectOnSmileyOrSkill(
    timerOn,
    skillLevel,
    dimensions,
    boardDispatch
  );
  BoardHooks.resetFlagsRemainingOnSkillChangeOrTimerOn(
    timerOn,
    skillLevel,
    boardDispatch
  );

  const setScrollBoardCallback = useCallback(
    () => boardDispatch({ type: "TOGGLE SCROLLBOARD" }),
    []
  );

  // Mixed!
  BoardHooks.revealFlipperEffect({
    mines,
    flippers,
    numbers,
    globalDispatch,
  ...
  const handleScroll = (_) =>
    boardDispatch({
      ...
      payload: { newColorDelay: event.deltaY },
    });
